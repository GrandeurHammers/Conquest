...
for (var i = 0; i < pointToLetter.length; i++) {
    ...
    result += `\n\tcreateInWorldText(getAllPlayers() if not powerPlayActive else null, "${pointToLetter[i]}\r\n", zoneLocations[${i}] + vect(0,2,0), 3, Clip.NONE, WorldTextReeval.VISIBILITY_STRING_AND_COLOR, zone${pointToLetter[i]}MainColor, SpecVisibility.DEFAULT)`;
    // Normal percentage indicator
    result += `\n\tcreateInWorldText(getAllPlayers() if not powerPlayActive else null, " \r\n{0}%    ".format(floor(abs(zone${pointToLetter[i]}Progress))) if abs(zone${pointToLetter[i]}Progress) > 0 else "", zoneLocations[${i}] + vect(0,2,0), 3, Clip.NONE, WorldTextReeval.VISIBILITY_STRING_AND_COLOR, zone${pointToLetter[i]}AltColor, SpecVisibility.DEFAULT)`;
    // Warning signs
